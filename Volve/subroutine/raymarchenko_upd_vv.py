...
class RayleighMarchenko_upd_vv():
    ...
    """
    def __init__(self, VZplus, VZminus, dt=0.004, nt=None, ds=1.,
    ...
    def apply_onepoint_upd(self, travsrc, FirstVSmr, G0=None, nfft=None, rtm=False, greens=False,
        ...
        G0 : :obj:`numpy.ndarray`, optional
            ...
            (if None, create arrival using ``travsrc``)
        ...
        **kwargs_solver
        ...
        iFirstVSmr_off = np.round(FirstVSmr_off / self.dt).astype(np.int64)
        travsrc_off = travsrc - self.toff
        itravsrc_off = np.round(travsrc_off / self.dt).astype(np.int64)
        ...
        if rtm:
            ...
            p0_minus = p0_minus.reshape(self.nt2, self.nr).T
        ...
        f1_inv = f1_inv.reshape(2 * self.nt2, self.ns)
        ...
        if greens:
            ...
            g_inv = g_inv.reshape(2 * self.nt2, self.nr)
        ...
        if rtm and greens:
            return f1_inv_minus, f1_inv_plus, p0_minus, g_inv_minus, g_inv_plus, G0
        elif rtm:
    ...
    def apply_multiplepoints_upd_vv(self, travsrc, FirstVSmr, G0=None, nfft=None,
        ...
        **kwargs_solver
        ...
        for ir in range(self.ns):
            for ivs in range(nvs):
                ws[ir, ivs, :int(itravsrc_off[ir, ivs])] = 1
        for ir in range(self.nr):
            for ivs in range(nvs):
                wr[ir, ivs, :int(iFirstVSmr_off[ir, ivs])] = 1
        ...
        Vzd1op = MDC(self.VZplus_fft, self.nt2, nv=nvs, dt=self.dt, dr=self.ds,
                     ...
                     saveGt=self.saveVt, prescaled=self.prescaled,
                     usematmul=usematmul)
        Wxrop = Diagonal(wr.transpose(2, 0, 1).flatten())
        ...
        if G0 is None:
            if self.wav is not None and nfft is not None:
                G0 = self.ncp.zeros((self.ns, nvs, self.nt), dtype=self.dtype)
                for ivs in range(nvs):
                ...
                raise ValueError('wav and/or nfft are not provided. '
                                 'Provide either G0 or wav and nfft...')
        fd_plus = self.ncp.concatenate((self.ncp.zeros((self.nt - 1, self.ns, nvs),
                                                 dtype=self.dtype),
                                  self.ncp.flip(G0, axis=-1).transpose(2, 0, 1)))

        # Run standard redatuming as benchmark
        if rtm:
            p0_minus = Vzuop * fd_plus.flatten()
            p0_minus = p0_minus.reshape(self.nt2, self.nr,
                                        nvs).transpose(1, 2, 0)

        # Create data and inverse focusing functions
        dinp = self.ncp.concatenate((self.ncp.zeros((self.nt2, self.ns, nvs),
                                              self.dtype),
                               fd_plus), axis=0)
        d = -Dop * dinp.ravel()
        ...
        f1_inv = f1_inv.reshape(2 * self.nt2, self.ns, nvs)
        f1_inv_tot = \
            f1_inv + self.ncp.concatenate((self.ncp.zeros((self.nt2, self.ns, nvs),
        ...
        if greens:
            # Create Green's functions
            g_inv = Gop * f1_inv_tot.flatten()
            g_inv = g_inv.reshape(2 * self.nt2, self.nr, nvs)
            g_inv_minus = -g_inv[:self.nt2].transpose(1, 2, 0)
            g_inv_plus = self.ncp.flip(g_inv[self.nt2:], axis=0).transpose(1, 2, 0)

        # Bring back to time axis with negative part
        f1_inv_minus = self.ncp.fft.ifftshift(f1_inv_minus, axes=-1)
        f1_inv_plus = self.ncp.fft.ifftshift(f1_inv_plus, axes=-1)
        if rtm:
            p0_minus = self.ncp.fft.ifftshift(p0_minus, axes=-1)
        if greens:
            g_inv_minus = self.ncp.fft.ifftshift(g_inv_minus, axes=-1)
            g_inv_plus = self.ncp.fft.ifftshift(g_inv_plus, axes=-1)
