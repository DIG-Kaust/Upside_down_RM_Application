...
from subroutine.raymarchenko_upd import RayleighMarchenko_upd
from subroutine.raymarchenko_upd_vv import RayleighMarchenko_upd_vv

def _imaging_depth_level(vsx, vsz, r, s, dr, ds, dt, nt, vel, 
                         toff, nsmooth, wav, wav_c, niter, nfmax,
                         igaths, nalpha,
                         data, kind='mck', jt=1, ivsz=None, trav_s2=None, trav_mr2=None, verb=False):
    ...
    if kind in ['mck', 'nmck']:
        ...
        rm = RayleighMarchenko_upd(data['Vzd'], data['Vzu'], dt=dt, ds=ds,
                                  nfmax=nfmax, wav=wav, toff=toff,
                                  nsmooth=nsmooth, saveVt=False, prescaled=False)
        f1_inv_minus, f1_inv_plus, p0_minus, g_inv_minus, g_inv_plus = \
    ...
    elif kind in ['rmck_upd_vv', ]:
        directVSs =  trav_s2[:,ivsz,:].T
        FirstVSmr =  trav_mr2[:,ivsz,:].T
        ...
        rm = RayleighMarchenko_upd_vv(data['Vzd'], data['Vzu'], dt=dt, ds=ds,
                                  nfmax=nfmax, wav=wav, toff=toff,
                                  nsmooth=nsmooth, saveVt=False, prescaled=False)
        _, _, p0_minus, g_inv_minus, g_inv_plus = \
            rm.apply_multiplepoints_upd_vv(directVSs, FirstVSmr, G0=G0src,
                                       rtm=True, greens=True,
                                       dottest=False,
                                       **dict(iter_lim=niter,
                                              show=False))
...
def MarchenkoImaging(vsx, vsz, r, s, dr, ds, dt, nt, vel,
                     toff, nsmooth, wav, wav_c, nfmax, igaths, nalpha, jt,
                     data, kind='rmck_upd', niter=10, nproc=1, nthreads=1, trav_s2=None, trav_mr2=None):
    """Marchenko imaging
    ...
    Parameters
    kind=
    'mck' or 'nmck': Marchenko method
    
    'rmck': Raylei Marchenko method
    
    'rmck_upd': Upside-down Raylei Marchenko method
    
    'rmck_upd_vv': Upside-down Raylei Marchenko method to deal with the variable velocity model
    ----------
    ...
    if kind == 'rmck_upd_vv':
        ...
        if nproc == 1:
            results = [_imaging_depth_level(vsx, vsz[ivsz], r, s, dr, ds, dt, nt, vel,
                                            toff, nsmooth, wav, wav_c, niter, nfmax,
                                            igaths, nalpha, data, kind=kind, jt=jt, ivsz=ivsz, 
                                            trav_s2=trav_s2, trav_mr2=trav_mr2)
                       for ivsz in range(nvsz)]
        else:
            print('nvsz = ',nvsz)
            pool = mp.Pool(processes=nproc)
            results = pool.starmap(_imaging_depth_level, [(vsx, vsz[ivsz], r, s, dr, ds, dt, nt, vel,
                                                        toff, nsmooth, wav, wav_c, niter, nfmax,
                                                        igaths, nalpha, data, kind, jt, ivsz, 
                                                        trav_s2, trav_mr2) for ivsz in range(nvsz)])    
    ...
    #f1_inv_minus= np.concatenate([results[ivsz][4][:, np.newaxis, :] for ivsz in range(nvsz)], axis=1)
    #f1_inv_plus= np.concatenate([results[ivsz][5][:, np.newaxis, :] for ivsz in range(nvsz)], axis=1)
    #p0_minus= np.concatenate([results[ivsz][6][:, np.newaxis, :] for ivsz in range(nvsz)], axis=1)
    #g_inv_minus= np.concatenate([results[ivsz][7][:, np.newaxis, :] for ivsz in range(nvsz)], axis=1)
    #g_inv_plus= np.concatenate([results[ivsz][8][:, np.newaxis, :] for ivsz in range(nvsz)], axis=1)
