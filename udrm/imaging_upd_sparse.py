...
from subroutine.raymarchenko_upd_sparse import RayleighMarchenko_upd


def _imaging_depth_level(vsx, vsz, Slid, r, s, dr, ds, dt, nt, iava, nrava, vel,
    ...
    if kind in ['mck', 'nmck']:
        ...
        f1_inv_minus, f1_inv_plus, p0_minus, g_inv_minus, g_inv_plus = \
            rm.apply_multiplepoints_upd(directVSs, FirstVSmr, Slid, niter,G0=G0src, 
    ...
    print('max of g_inv_plus is:',g_inv_plus.max())
    print('max of g_inv_minus is:',g_inv_minus.max())
    print('max of G0 is:',G0.max())
    print('max of p0_minus is:',p0_minus.max())
    print('max of iss is:',G0.max())
    print('max of imck is:',p0_minus.max())
    
    return iss, imck, ass, amck, f1_inv_minus, f1_inv_plus

def MarchenkoImaging_upd(vsx, vsz, Slid, r, s, dr, ds, dt, nt, iava, nrava, vel,
    ...
    t0 = time.time()
    if nproc == 1:
        results = [_imaging_depth_level(vsx, vsz[ivsz],Slid, r, s, dr, ds, dt, nt, iava, nrava, vel,
                                        toff, nsmooth, wav, wav_c, niter, nfmax,
                                        igaths, nalpha, data,
        ...
        results = pool.starmap(_imaging_depth_level, [(vsx, vsz[ivsz], Slid, r, s, dr, ds, dt, nt, iava, 
